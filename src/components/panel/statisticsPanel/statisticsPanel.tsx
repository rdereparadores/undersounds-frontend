...
import { BarChart, Bar, XAxis, YAxis, TooltipProps} from "recharts"
        ...
        <Card className="w-full overflow-hidden border-border/10 shadow-lg bg-gradient-to-br from-blue-50 to-white dark:from-blue-950/30 dark:to-blue-900/10 dark:bg-blue-950">
            <CardHeader className="pb-0 bg-blue-500/5 dark:bg-blue-800/10 backdrop-blur-sm p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <Badge
                        variant="outline"
                        className="px-3 py-1.5 bg-blue-500 text-white border-blue-400 hover:bg-blue-600 transition-colors"
                    >
                        {activeData.period}
                    </Badge>
                </div>
            ...
            <CardContent className="p-4 sm:p-6">
                <Tabs
                    ...
                    className="w-full"
                >
                    <TabsList className="grid grid-cols-3 gap-2 mb-6 bg-blue-100/50 dark:bg-blue-900/30 p-1 rounded-md">
                        <TabsTrigger
                            value="purchases"
                            className="flex items-center gap-2 hover:bg-blue-200/50 dark:hover:bg-blue-800/50 data-[state=active]:bg-blue-600 data-[state=active]:text-white rounded-md p-2 transition-colors"
                        >
            ...
            <CardFooter className="bg-blue-100/30 dark:bg-blue-800/20 border-t border-blue-200/50 dark:border-blue-700/30 px-4 sm:px-6 py-4">
...
function StatisticsContent({ data, type }: StatisticsContentProps) {
    ...
    // Ajustamos la paleta de colores a tonos de azul
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Contenedor con scroll si el gráfico excede el ancho */}
            <div className="mt-4 transition-all duration-500">
                <ChartContainer config={data.chartConfig || {}} className="h-[300px] sm:h-[350px] w-full">
                    {/*
            Usamos un contenedor con un ancho fijo (por ejemplo, 600px) y un overflow horizontal,
            de modo que si la pantalla es más pequeña, se pueda hacer scroll sin que se corte.
          */}
                    <div className="relative w-full max-w-full overflow-x-auto" style={{ height: "100%" }}>
                            ...
                            >
                                ...
                                <YAxis
                                    ...
                                    tick={({ x, y, payload }: { x: number; y: number; payload: { value: string } }) => {
                                        ...
                                        const shouldBeBold = ["CD cancion", "CD album", "Pop", "Rock", "Luna Nova"].some((str) =>
                                            payload.value.includes(str),
                                        )
                                        return (
                                            <text
                                                x={x}
